...
import com.blankScreen.standby.settings.NoCloseOptionSelectedNotification;
import com.blankScreen.standby.utils.OnHideFinishedListener;
import com.blankScreen.standby.utils.OnSwipeListener;
import com.blankScreen.standby.utils.OverlayView;
...
public class OverlayService extends AccessibilityService {
    ...
    public static final int pixelOffset = 400;
    public static byte state = Constants.Overlay.State.UNSET;
    private DisplayMetrics screenDimension;
    OverlayView view;//rendering black screen

    static WindowManager windowManager;
    static WindowManager.LayoutParams layoutParams;
    private final int flags = WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE |
    ...
    private void init() {
        ...
        getApplication().setTheme(R.style.Theme_STANDBY);

        // Get display dimensions
        screenDimension = getResources().getDisplayMetrics();
    ...
    private boolean hide() {
        // Check for the right state. The hiding action can be started while the overlay is only
        // visible or
    ...
    private void show() {
        ...
        if (enabled_options < 1) {
            NoCloseOptionSelectedNotification notification =
                    new NoCloseOptionSelectedNotification(this);
            notification.drop();
    ...
    private void addView() {
        ...
        if (state == Constants.Overlay.State.INITIALIZED ||
            ...
            layoutParams.width = screenDimension.widthPixels + 2 * pixelOffset;
            layoutParams.height = screenDimension.heightPixels + 2 * pixelOffset;
    ...
    private void writeOverlayShowingPref(boolean value) {
        getSharedPreferences(Constants.Preferences.PREFERENCE_NAME, MODE_PRIVATE).edit()
                .putBoolean(Constants.Preferences.IS_OVERLAY_SHOWING, value).apply();
        Log.i(getClass().getName(), "Successfully wrote preference " +
                Constants.Preferences.IS_OVERLAY_SHOWING + " to " + (value ? "true":"false"));
    }
    ...
    private void writeServiceRunningPref(boolean value) {
        running = value;
        getSharedPreferences(Constants.Preferences.PREFERENCE_NAME, MODE_PRIVATE).edit()
                .putBoolean(Constants.Preferences.IS_SERVICE_RUNNING, value).apply();
        Log.i(getClass().getName(), "Successfully wrote preference " +
                Constants.Preferences.IS_SERVICE_RUNNING + " to " + (value ? "true":"false"));
    }

    private boolean getInvertOverlayColorPref() {
        return PreferenceManager.getDefaultSharedPreferences(this)
                .getBoolean("setting_invert_overlay_color", false);
    }
    ...
    private void hide_immediately() {
        ...
        if (state == Constants.Overlay.State.VISIBLE ||
            ...
            Log.e(getClass().getName(), "Overlay is not in required state. " +
                    "Cancel hiding. Overlay is in state " + Constants.Overlay.getStateName(state));
        }
    }
    ...
    private boolean getStartOnBootPref() {
        return PreferenceManager.getDefaultSharedPreferences(this)
                .getBoolean("setting_start_on_boot", false);
    }

    private boolean getShowNotificationPref() {
        return PreferenceManager.getDefaultSharedPreferences(this)
                .getBoolean("setting_show_notification", false);
    }

    private boolean getUseWakeLockPref() {
        return PreferenceManager.getDefaultSharedPreferences(this)
                .getBoolean("setting_use_wake_lock", true);
    }
